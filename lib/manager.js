...
Manager.prototype.getDependencies = function() {
  if (!this.dependencies_) {
    ...
    var visit = function(namespace) {
      if (!visited.hasOwnProperty(namespace)) {
        visited[namespace] = true;
        var script = providesLookup[namespace];
        ...
        if (!added.hasOwnProperty(script.name)) {
          added[script.name] = true;
          dependencies.push(script);
        }
      }
    };

    Object.keys(providesLookup).forEach(visit);
