...
var Manager = exports.Manager = function Manager(options) {
  ...
  // use built-in closure-library by default
  if (options.closure) {
    var closure = path.join(__dirname, '..', 'bower_components',
        'closure-library');
    paths.unshift(path.join(closure, 'closure', 'goog', '**', '*.js'),
  ...
  process.nextTick(this.processPaths_.bind(this, function(err) {
    if (!err) {
      this.startWatch_();
    }
  }.bind(this)));
 ...
 * @private
 */
Manager.prototype.processPaths_ = function(done) {
  var self = this;
  async.waterfall([
    function(callback) {
