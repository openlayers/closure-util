...
var Server = exports.Server = function Server(config) {
   ...
   * @private
   */
  this.socket_ = config.hasOwnProperty('socket') ? config.socket : true;
 ...
 * @param {function=} opt_callback Callback added as a listener for the
 *     'listening' event.
 * @return {http.Server} The underlying HTTP server.
 */
Server.prototype.listen = function(opt_port, opt_callback) {
  ...
  if (this.socket_) {
    ...
    io.sockets.on('connection', function(socket) {
      ...
      manager.getErrors().forEach(emit);
      manager.on('error', emit);
      manager.on('update', function(script) {
        socket.emit('update', script && script.name);
      });
    });
  }
  return server.listen(port, opt_callback);
};
